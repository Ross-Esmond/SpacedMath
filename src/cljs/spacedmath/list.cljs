...
(def math-list
  (atom
    [
      ...
      [:equal [:fn \f \t] [:add [:mult 2 [:power \t 3]] [:mult -3 [:power \t 2]] [:mult -4 \t]]]
      [:equal [:fn \f \t] [:add [:mult 1.4 [:power \t 5]] [:mult -2.5 [:power \t 2]] 6.7]]
      [:equal [:fn \g \x] [:add [:power \x 2] [:mult -2 [:power \x 3]]]]
...
(def detailed-math-list
  [[[:equal [:fn \f \x] [:exp \x]] #{:exp} [:exp \x]]
   ...
   [[:equal [:fn \g \x] [:mult [:add \x [:mult 2 [:root \x 2]]] [:exp \x]]]
    #{:product :exp :add :power :scaler}
    [:add
     [:mult [:add 1 [:power "x" [:div -1 2]]] [:exp "x"]]
